...
public class DeveloperBugListUI
{
    ...
    public static Scene create (Stage stage, int uID)
    {
        ...
        String[] strArray = ctrl.getAssignedBugs(uID);
        for(int i = 0; i < strArray.length; i++)
        {// Fill the "list" TextArea with bug records
            if(i==0) {list.appendText(strArray[i]);}       
            else {list.appendText("\n" + strArray[i]);}
        }
        
        list.setEditable(false); // Don't alow users to edit the list
